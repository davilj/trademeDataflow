...
/**
 * A pipeline that run thought all listing, extract the bids (listing with bids), 
 * group by cat and cat3, 
 * aggregate over month, month:day and month:day:hour
 * 
 * @author daniev
 *
 */
public class Aggregator {
	...
	public static Pipeline createPipeline(AggregateOptions dailySalesOptions) {
		...
		PCollectionTuple tuples = p
				.apply(TextIO.Read.withCompressionType(TextIO.CompressionType.AUTO).from(dailySalesOptions.getInputFile()))
